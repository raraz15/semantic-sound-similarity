...
def plot_av_label_based_map_at_15(model, eval_dir=EVAL_DIR, D=0.25, 
                   save_fig=False, save_dir=FIGURES_DIR):
    """Takes a model name and plots the mAP@k for all the variations of the model."""
    ...
    for search in searches:
        for variation_path in variation_paths:
            map_path = os.path.join(variation_path, search, "av_label_based_mAP_at_15.txt")
    ...
    if save_fig:
        os.makedirs(save_dir, exist_ok=True)
        fig_path = os.path.join(save_dir, f"{model}-av_label_based_mAP@15.png")
        print(f"Saving figure to {fig_path}")
        fig.savefig(fig_path)
    plt.show()
...
if __name__=="__main__":
    ...
    plot_av_label_based_map_at_15(args.model, save_fig=True, save_dir=args.save_dir)
